...
def make_get_request(subreddit, after):
    ...
    if after:
        playload = {'after': after, 'limit': '100'}
    else:
        playload = {'limit': '100'}
    header = {'user-agent': 'one-dope-boy', 'over18': 'yes'}
    response = requests.get(
        url, headers=header, params=playload, allow_redirects=False
    )
    return response
...
def count_words(subreddit, word_list, after=None):
    """
    prints the count of top the hot posts for subreddit
    """
